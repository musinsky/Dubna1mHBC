...
void np_OLD2()
{
  ...
  bizard0->SetName("bizard");
  bizard0->SetLineColor(kBlack);
  TGraphErrors *bonner0  = new TGraphErrors();
  bonner0->SetName("bonner");
  bonner0->SetLineColor(kBlue);
  TGraphErrors *shepard0 = new TGraphErrors();
  shepard0->SetName("shepard");
  shepard0->SetLineColor(kGreen);
  ...
  /*
    ...
    bizards->Draw("P");
    bonners->Draw("P");
    ge = (TGraphErrors *)bizards->GetListOfGraphs()->At(14);
    return;
  */
  ...
  bonner0->Draw("P");
  shepard0->Draw("P");
  Double_t x, y, e = bizard0->GetErrorY(14);
  bizard0->GetPoint(14, x, y);
  Printf("p = %5.3f, sig(0) = %g +- = %g", x, y, e);
  
  Double_t impuls = 3.35/2.;
  TF1 *fe = new TF1("fe", "[0]*TMath::Exp([1]*x)");
  fe->SetParameter(0, 100);
  fe->SetLineColor(bizard0->GetLineColor());
  fe->SetLineWidth(1);
  bizard0->Fit("fe", "Q", "", impuls-0.27, impuls+0.27); // +- 5 points
  Double_t p0 = fe->GetParameter(0), p0Err = fe->GetParError(0);
  Double_t p1Err = fe->GetParError(1);
  TVirtualFitter *fitter = TVirtualFitter::GetFitter();
  Double_t cov = fitter->GetCovarianceMatrixElement(0, 1); // = (1, 0)
  Double_t error = (p0Err*p0Err)/(p0*p0) + (impuls*impuls*p1Err*p1Err);
  error += 2.*cov*(impuls/p0);
  error = fe->Eval(impuls)*TMath::Sqrt(error);
  Printf("p = %5.3f, sig(0) = %g +- = %g", impuls, fe->Eval(impuls), error);
  
  // Draw final figure
  gROOT->Reset();
  gROOT->SetStyle("Plain");
  TCanvas *c = new TCanvas("figBizard","Bizaard cross section");
  TGraphErrors *bc = (TGraphErrors *)bizard0->DrawClone("AP");
  Double_t rmin, rmax;
  fe->GetRange(rmin, rmax);
  bc->GetXaxis()->SetLimits(rmin, rmax);
  bc->SetMaximum(fe->Eval(rmin)*1.1);
  bc->SetMinimum(0);
  bc->SetMarkerStyle(7);
  bc->GetYaxis()->SetTitle("#frac{d#sigma}{dt}#cbar_{t = 0} [ #frac{mb}{(GeV/c)^{ 2}} ]");
  bc->GetYaxis()->CenterTitle();
  bc->GetXaxis()->SetTitle("P_{LAB} [ GeV/c ]");
}
